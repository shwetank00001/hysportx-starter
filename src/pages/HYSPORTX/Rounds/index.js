...
const index = () => {
      ...
      },
      {
        ...
        Cell: cellProps => {
          return (
            <img
              src={`${baseUrl}${
                cellProps.row.original ? cellProps.row.original.image : ""
              }`}
              alt=""
              width={50}
              height={50}
  ...
  const onClickDelete = data => {
    setCompetiton(data), setDeleteModal(true)
  }
  const handleDelete = () => {
    setDeleteModal(false), dispatch(removeRoundRequest(competition.id))
  }
  return (
    <React.Fragment>
      <DeleteModal
        text={"Are you Sure you want to Delete the Competition list ?"}
        show={deleteModal}
