...
import { Row, Modal, ModalHeader, ModalBody, ModalFooter, Form, Label, Input, FormFeedback,Button } from 'reactstrap'
import {useSelector, useDispatch } from "react-redux";
import {addCompetitionRequest} from '../../../store/actions'
const AddFormModal = () => {
    ...
    return (
        <React.Fragment>
            <Link to="#" className="btn btn-primary" onClick={e => { setCompetitionModal(true); }}  ><i className="mdi mdi-plus me-1" />Create Competition</Link>
            <Modal isOpen={competitionModal} toggle={() => { setCompetitionModal(); }} id="activity"size="lg">
                <div className="modal-content">
                    <Form className="form-horizontal" onSubmit={(e) => { e.preventDefault(); validation.handleSubmit(); return false;}} encType="multipart/form-data">
                        <ModalHeader toggle={() => setCompetitionModal()} id="activitiesLabel" className="modal-header"> Create Competition</ModalHeader>
                                ...
                                <div className="row mb-3">
                                    ...
                                    <div className="col-lg-9 col-md-9">
                                        <Input type="file" name='image' className="form-control" id='image'onBlur={validation.handleBlur}
                                            onChange={(event) => {const file = event.target.files[0]; validation.setFieldValue('image', event.currentTarget.files[0]);
                                                 if (file) {
                                                    const fileDetails = {
                                                        name: file.name,
                        ...
                        <ModalFooter>
                            <div className="text-end">
                                <button type='submit' className="btn btn-success me-1">Save competition <i className="bx bx-send align-middle"></i></button>
                            </div>
                        </ModalFooter>
                    </Form>
                </div>
