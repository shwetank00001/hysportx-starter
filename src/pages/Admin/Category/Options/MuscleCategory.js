...
import {muscleListRequest,addMuscleRequest,deleteMuscleRequest,} from "../../../../store/muscle/actions"
...
function MuscleCategory() {
  ...
  useEffect(() => {dispatch(muscleListRequest())}, [dispatch])
  const muscles = muscleDispatch.muscle ? muscleDispatch.muscle.map(item => item): []

  const handleDelete = muscleId => {dispatch(deleteMuscleRequest(muscleId)) }
  const addMuscleHandler = () => { dispatch(addMuscleRequest({name: muscleName,description: muscleDescription,}))
  ...
  return (
 <React.Fragment>
  <DeleteModal text={'Are you Sure you want to Delete the Muscle list ?'} show={deleteModal} onDeleteClick={handleDelete} onCloseClick={() => setDeleteModal(false)} />
  ...
  <Modal  isOpen={modal} autoFocus={true} centered={true} toggle={() => { setModal(!modal) }}>
        <div className="modal-content">
          <ModalHeader toggle={() => { setModal(!modal) }}> Manage Muscle</ModalHeader>
          <ModalBody>
            <form>
              <div className="mb-3">
                <Input type="text" className="form-control" placeholder="Muscle Name" value={muscleName}onChange={e => setMuscleName(e.target.value)} />
              </div>
              <div className="mb-3">
                <Input type="text" className="form-control" placeholder="Muscle Description" value={muscleDescription} onChange={e => setMuscleDescription(e.target.value)}/>
              </div>
            </form>
          </ModalBody>
          <ModalFooter className="">
            ...
            <Col className="text-center">
              <Button className="col-sm-12 btn-soft-info" type="button" color="success" onClick={addMuscleHandler}> Save  <i className="bx bx-send align-middle"></i></Button>
            </Col>
          </ModalFooter>
        </div>
      </Modal>
